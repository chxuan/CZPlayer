...
void Widget::createUI()
{
	...
	openFileAction->setIcon(QIcon(":/images/addMusic.png"));
...
void Widget::dragEnterEvent(QDragEnterEvent *event)
{
    ...
    foreach (QUrl url, urls)
    {
		...
		if (fileInfo.isFile())	//可拖进文件
		{
			...
			if (strSuffixName == "wav" || strSuffixName == "WAV" || strSuffixName == "m3u" || strSuffixName == "M3U" || strSuffixName == "pls" || strSuffixName == "PLS")
			{
				event->accept();
			}
			else
...
void Widget::paintEvent(QPaintEvent *event)
{
    ...
    backgroundImage.load(":/images/mainBg2.png");
...
//ascii字符转换成Unicode字符
int Widget::astr_ustr( char *ansistr, WCHAR *unicodestr )
{
	int result = 0;
	try
	{
...
void Widget::slot_SharedCurrentMusic( QString strMusicName )
{
	...
	if (sharedDialog->isHidden())
	{
		...
		{
			sharedDialog->initSharedDialog(":/images/albumImage.png", strMusicName, strAlbumUrlPath);
		}
...
void Widget::play( const QString &strCurrentFilePath, const QString &strCurrentMusicName, const int &nCurrentMusicRow, const QString &strCurrentMusicTime )
{
	...
	m_engine->loadFile(strCurrentFilePath);			//播放引擎加载文件
	m_engine->slot_StartPlayback();					//开始播放
	this->setState(Play);								//设置状态
