...
LrcSettingsPage::LrcSettingsPage(QWidget *parnet) : m_parent(parnet)
{
    ...
    QFontDatabase font;
    //QStringList fontList = font.families(QFontDatabase::SimplifiedChinese);//获取系统能显示中文的字体
    QStringList fontList = font.families();//获取系统能显示中文的字体
    ...
    connect(backToButton, SIGNAL(clicked()), this, SLOT(slot_BackToColor()));//恢复默认方案
    connect(yuSheButton, SIGNAL(clicked()), this, SLOT(slot_YuSheColor()));//预设方案
...
void LrcSettingsPage::slot_ChangeColor()
{
    ...
    QColor color = QColorDialog::getColor(QColor(red, green, blue), this, tr("选择颜色"), QColorDialog::DontUseNativeDialog);
    if (color.isValid())
    {
...
void LrcSettingsPage::slot_ChangeYuSheColor()
{
    ...
    else if (action == haiYangBlue)//海洋蓝
    {
    ...
    else if (action == kuXuanBlack)//酷炫黑
    {
    ...
    else if (action == qiuSeYellow)//秋色黄
    {
    ...
    else if (action == langManPurple)//浪漫紫
    {
